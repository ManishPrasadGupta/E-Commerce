...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        setProduct(await apiClient.getProduct(id));
      } catch (err) {
  ...
  if (error || !product) {
    return (
      <div className="alert-error max-w-md mx-auto my-8 flex items-center gap-2 p-4 bg-red-100 text-red-700 rounded shadow">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16">
        ...
        <div
          id="product-carousel"
          className="relative w-full rounded-2xl shadow-xl bg-gradient-to-br from-white/70 via-blue-100/30 to-cyan-50/30 p-6"
        >
            ...
            <div className="absolute z-30 flex -translate-x-1/2 bottom-6 left-1/2 space-x-2">
              {images.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full border-2 border-blue-400 transition-all
                    ${
                      activeIndex === index
                        ? "bg-blue-500"
                        : "bg-white opacity-60"
          ...
          {images.length > 1 && (
            <>
              <button
                onClick={prevSlide}
                type="button"
                className="absolute top-1/2 left-2 z-30 -translate-y-1/2 flex items-center justify-center h-10 w-10 rounded-full bg-white/80 hover:bg-blue-50 shadow transition"
              ...
              <button
                onClick={nextSlide}
                type="button"
                className="absolute top-1/2 right-2 z-30 -translate-y-1/2 flex items-center justify-center h-10 w-10 rounded-full bg-white/80 hover:bg-blue-50 shadow transition"
        ...
        <div className="space-y-8">
          ...
          {/* Radix RadioGroup for Variant Selection */}
          <div className="space-y-4">
            ...
            <RadioGroup.Root
              ...
              onValueChange={(val) => {
                const found = product?.variants?.find((v) => v.type === val);
              ...
              {product?.variants?.map((variant) => (
                <RadioGroup.Item
                  ...
                  className={`rounded-lg border transition-all cursor-pointer
                    bg-white shadow-sm px-4 py-4 flex justify-between items-center
